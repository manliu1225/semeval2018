...
#bfLeft = "(♥|0|o|°|v|\\$|t|x|;|\u0ca0|@|ʘ|•|・|◕|\\^|¬|\\*)".encode('utf-8')
bfLeft = u"(♥|0|[oO]|°|[vV]|\\$|[tT]|[xX]|;|\u0ca0|@|ʘ|•|・|◕|\\^|¬|\\*)".encode('utf-8')
print('type bfleft {}'.format(type(bfLeft)))
bfCenter = r"(?:[\.]|[_-]+)"
bfRight = r"\2"
s3 = r"(?:--['\"])"
s4 = r"(?:<|&lt;|>|&gt;)[\._-]+(?:<|&lt;|>|&gt;)"
...
# bfLeft, bfCenter, bfRight,s3, s4, s5 = map(lambda x : str(x), [bfLeft, bfCenter, bfRight,s3, s4, s5])
basicface = "(?:" +bfLeft+bfCenter+bfRight+ ")|" +s3+ "|" +s4+ "|" + s5

eeLeft = ur"[＼\\ƪԄ\(（<>;ヽ\-=~\*]+".encode('utf-8')
eeRight= u"[\\-=\\);'\u0022<>ʃ）/／ノﾉ丿╯σっµ~\\*]+".encode('utf-8')
eeSymbol = ur"[^A-Za-z0-9\s\(\)\*:=-]".encode('utf-8')
# eeLeft, eeRight, eeSymbol = map(lambda x : str(x), [eeLeft, eeRight, eeSymbol])
...
AtMention = u"[@＠][a-zA-Z0-9_]+".encode('utf-8')
...
re.compile(emoticon)
Protected  = re.compile(regex_or(Hearts,
        url,
        Email,
        timeLike,
        #numNum,
...
def simpleTokenize(text):
    ...
    print('splitPunctText {}'.format(splitPunctText))
    ...
    print('badSpans {}'.format(badSpans))
    ...
    print('splitGoods {}'.format(splitGoods))
    ...
    #zippedStr = splitStr
    print('simpleTokenize {}'.format(zippedStr))
    return zippedStr
...
def squeezeWhitespace(input):
    print('squeezeWhitespce input {}'.format(input))
    print('squeezeWhitespace {}'.format(Whitespace.sub(" ", input).strip()))
...
def normalizeTextForTagger(text):
    text = text.replace("&amp;", "&")
    print('normalizeTextForTagger {}'.format(text))
    text = HTMLParser.HTMLParser().unescape(text.decode('utf8'))
    print('HTMLParser {}'.format(text))
    return text
...
def tokenizeRawTweetText(text):
    ...
    print('tokenizeRawTweetText {}'.format(tokens))
    return tokens
